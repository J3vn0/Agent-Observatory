...
| **기능 관리** | 에이전트별 관찰·채택 스킬, 플러그인, 훅, MCP와 토큰 기반 채택 관리 |
...
## 강제 비용 봉투 안에서 기능 채택하기

**기능 관리** 화면은 혼동하기 쉬운 설치됨, 관찰됨, 제안됨, 채택됨,
활성 상태를 구분합니다. “관찰됨”은 귀속 메타데이터나 명시적 로컬
매니페스트가 근거를 제공할 때만 표시하며 대화문에서 추측하지 않습니다.

**채택**을 누르면 먼저 10분 동안 유효한 계획을 만듭니다. 로컬 데몬은
다음 항목을 검증합니다.

- 지침 지연 로딩과 중복 호출 억제
- 컨텍스트, 출력, 시간·일일 호출, 실행당·일일 토큰 상한
- 최소 60분 예약 간격과 예약 빈도에서 계산한 호출 제한
- 권한 위험, 루프백 브라우저 출처, 명시적 승인
- 덮어쓰지 않는 고정 표시 안전 매니페스트 대상

승인된 계획은 <code>~/.agent-observatory/adoptions/</code> 아래에 에이전트와
기능별 로컬 관리 매니페스트를 하나씩 만듭니다. 생성 파일이 원본 해시와
같을 때만 되돌릴 수 있습니다. 이 매니페스트는 관리 의도와 안전 상한을
기록하며, 환경별 실제 런타임 활성화는 별도의 명시적 어댑터 단계입니다.
...
- [x] 설명 가능한 기능 유사도와 중복 억제
- [x] 승인 기반의 되돌릴 수 있는 승격 실행
- [x] 토큰 기반 기능 채택 계획과 해시 검증 되돌리기
- [x] 에이전트별 관찰·채택 기능 원장
- [ ] 환경별 실제 런타임 활성화 어댑터
- [ ] 추가 에이전트 환경 어댑터
- [ ] 읽기 전용 금융 관찰 팩
