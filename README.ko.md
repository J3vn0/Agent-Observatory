...
## 설명하고, 묶어서 보고, 안전하게 실행하기

발견한 에이전트, 스킬, 플러그인, 훅, MCP 서버는 허용된 로컬 메타데이터만
사용한 간단한 설명을 마우스 오버와 키보드 포커스로 보여줍니다.

- 같은 역할의 반복 서브에이전트 실행은 중복 노드 대신 하나로 집계합니다.
- Claude 귀속 메타데이터에서 `Deep Research` 같은 이름과 스킬을 판별합니다.
- 레지스트리 실행 전 대상 경로와 기존 파일 충돌 여부를 먼저 보여줍니다.
- 승인 시 공용 Markdown 또는 Codex TOML 정의를 새로 만들고 해시 검증 기반 되돌리기를 제공합니다.
